...
const updatedFriends = friends.map(fr => {
  if (fr.id === id) {
    return {...fr, married : !fr.married} //the ...
  } else {
    return fr;
  }
})
...
const getFilteredFriends = () => {
  ...
  return (
    <div className='app-friends container'>
      ...
      <FriendsList searchTerm = {searchTerm} changeStatus = {changeStatus} friends = {getFilteredFriends()}/> {/*first friends is the key and second is key in the prop object */}
    </div>
  )
}
